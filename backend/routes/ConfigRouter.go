...
func routeProduct(router *fiber.Router) {
	...
	(*router).Get("/:id", controller.GetProductById) // timf theo id

	(*router).Get("/category/:name_category", controller.GetProductsByCategory) // tim theo category

	(*router).Get("/issale/:is_sale", controller.GetProductsSale)

	(*router).Get("/name/:input_name", controller.GetProductByName)
}
