<template>
  <div class="container buffer-top">
    <Navigation />
    <h2 class="text-start  border-bottom">
      Tìm kiếm cho: <span class="redtext">{{ input_name }}</span>
    </h2>
    <div class="dropdown col-md-2">
      ...
      >
        <option value="1" selected>Sắp xếp</option>
...
export default {
  ...
  methods: {
    ...
    async goToOtherPage(index) {
      ...
      this.products2 = results.listProduct;
    },
    formatPrice(price) {
  ...
  async created() {
    const response = await fetch(
      `http://localhost:3000/api/product/name/${this.input_name}?limit=6&offset=0`
    );
    let results = await response.json();
    this.products = results.listProduct;
    this.products2 = results.listProduct;
